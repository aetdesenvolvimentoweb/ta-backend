...
} from "./application/use-cases/manage-repertoire.use-case";
...
} from "./infra/http/controllers/artist.controller";
...
} from "./infra/http/controllers/payment-account.controller";
...
if (env.MP_CLIENT_ID && env.MP_CLIENT_SECRET) {
  ...
  logger.warn(
    "Mercado Pago não registrado: MP_CLIENT_ID/MP_CLIENT_SECRET ausentes. Conexão de conta desabilitada."
  );
}

// 2. Use Cases
const createArtistUseCase = new CreateArtistUseCase(artistRepository, passwordHasher, logger);
const authenticateArtistUseCase = new AuthenticateArtistUseCase(
  artistRepository,
  passwordHasher,
  logger
);
...
const requestMusicUseCase = new RequestMusicUseCase(
  requestRepository,
  showRepository,
  songRepository,
  artistRepository,
...
const getShowRequestsUseCase = new GetShowRequestsUseCase(
  requestRepository,
  showRepository,
  logger
);
const markSongAsPlayedUseCase = new MarkSongAsPlayedUseCase(
  requestRepository,
  showRepository,
  logger
);
...
const validateAdminWhitelistUseCase = new ValidateAdminWhitelistUseCase(
  whitelistRepository,
  logger
);
const getAppMetricsUseCase = new GetAppMetricsUseCase(
  requestRepository,
  artistRepository,
  songRepository,
  logger
);
const getArtistMetricsUseCase = new GetArtistMetricsUseCase(
  requestRepository,
  showRepository,
  logger
);
const getPublicShowUseCase = new GetPublicShowUseCase(
  showRepository,
  artistRepository,
  songRepository,
  styleRepository,
...
const startPaymentConnectionUseCase = new StartPaymentConnectionUseCase(
  artistRepository,
  paymentRegistry,
  oauthStateStore,
  logger
);
const completePaymentConnectionUseCase = new CompletePaymentConnectionUseCase(
  artistRepository,
...
const disconnectPaymentAccountUseCase = new DisconnectPaymentAccountUseCase(
  artistRepository,
  credentialsRepository,
  logger
);
...
const createTipPaymentUseCase = new CreateTipPaymentUseCase(
  requestRepository,
  showRepository,
  artistRepository,
  credentialsRepository,
...
const finishShowUseCase = new FinishShowUseCase(
  showRepository,
  requestRepository,
  logger,
  refundTipPaymentUseCase
);
const cancelMusicRequestUseCase = new CancelMusicRequestUseCase(
  requestRepository,
  showRepository,
  logger,
  refundTipPaymentUseCase
...
const v1Router = new Elysia({ prefix: "/v1" })
  ...
  .use(
    musicRequestController(
      requestMusicUseCase,
      getShowRequestsUseCase,
      cancelMusicRequestUseCase,
      ...
      documentation: {
        ...
        tags: [
          ...
          { name: "Metrics", description: "Métricas financeiras do artista" },
          {
            ...
            description: "Conexão OAuth de conta no gateway de pagamento (RN14/RN15)",
          },
