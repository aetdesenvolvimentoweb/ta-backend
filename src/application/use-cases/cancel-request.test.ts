...
describe("CancelMusicRequest Use Case (RN05)", () => {
  test("deve cancelar um pedido pendente do próprio artista", async () => {
    ...
    const useCase = new CancelMusicRequestUseCase(
      reqRepo as any,
      showRepo as any,
      mockLogger as any
    );
  ...
  test("não deve cancelar pedido já tocado", async () => {
    ...
    await expect(useCase.execute({ requestId: "req-1", artistId: "artist-1" })).rejects.toThrow(
      "já foi tocado"
    );
  });
  ...
  test("deve rejeitar cancelamento por artista que não é dono do show", async () => {
    ...
    await expect(useCase.execute({ requestId: "req-1", artistId: "artist-2" })).rejects.toThrow(
      UnauthorizedError
    );
  });
  ...
  test("deve lançar NotFound quando pedido não existe", async () => {
    ...
    await expect(useCase.execute({ requestId: "nope", artistId: "a" })).rejects.toThrow(
      NotFoundError
    );
  });
});
