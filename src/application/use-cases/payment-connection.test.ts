...
class MockArtistRepo {
  ...
  async save(a: Artist) { this.artists.set(a.id, a); }
  async delete(id: string) { this.artists.delete(id); }
}

class MockCredsRepo {
  saved: any = null;
  deleted: string | null = null;
  async save(c: any) { this.saved = c; }
  async findByArtistId() { return null; }
  async deleteByArtistId(id: string) { this.deleted = id; }
}
...
describe("StartPaymentConnectionUseCase", () => {
  test("gera authorizeUrl com state + code_challenge e armazena verifier", async () => {
    ...
    const useCase = new StartPaymentConnectionUseCase(repo as any, new MockRegistry(gw) as any, store, mockLogger as any);
  ...
  test("falha quando o artista não existe", async () => {
    ...
    await expect(useCase.execute({
      artistId: "missing", gateway: 'mercado_pago', redirectUri: "http://cb"
...
describe("CompletePaymentConnectionUseCase", () => {
  test("troca code por tokens, salva credentials e conecta paymentAccount no artista", async () => {
    ...
    const useCase = new CompletePaymentConnectionUseCase(
      repo as any, creds as any, registry as any, store, mockLogger as any
    );

    const out = await useCase.execute({ code: 'auth-code', state: 'st-1', redirectUri: 'http://cb' });
  ...
  test("rejeita state inválido (CSRF/replay)", async () => {
    const repo = new MockArtistRepo();
    const useCase = new CompletePaymentConnectionUseCase(
      repo as any, new MockCredsRepo() as any,
      new MockRegistry(buildMockGateway()) as any,
      new InMemoryOAuthStateStore(),
      mockLogger as any
    );
    await expect(useCase.execute({ code: 'c', state: 'forged', redirectUri: 'x' }))
...
describe("DisconnectPaymentAccountUseCase", () => {
  test("limpa paymentAccount do artista e remove credenciais (idempotente)", async () => {
    ...
    const creds = new MockCredsRepo();
    const useCase = new DisconnectPaymentAccountUseCase(repo as any, creds as any, mockLogger as any);
  ...
  test("falha quando o artista não existe", async () => {
    const useCase = new DisconnectPaymentAccountUseCase(
      new MockArtistRepo() as any, new MockCredsRepo() as any, mockLogger as any
    );
