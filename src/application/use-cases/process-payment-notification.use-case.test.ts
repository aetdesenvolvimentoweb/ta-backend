...
class MockRequestRepo {
  ...
  async findByPaymentId(id: string) { return this.byPaymentId.get(id) ?? null; }
...
describe("ProcessPaymentNotificationUseCase", () => {
  test("atualiza status para 'approved' quando gateway retorna approved", async () => {
    ...
    showRepo.shows.set("show-1", new Show("show-1", "artist-1", new Date(), new ShowDuration(4), "active"));

    const useCase = new ProcessPaymentNotificationUseCase(
      reqRepo as any, showRepo as any, credsRepo as any,
      new MockRegistry(buildMockGateway("approved")) as any, mockLogger as any
    );
  ...
  test("é idempotente — ignora quando status já é o mesmo", async () => {
    ...
    reqRepo.save = async (r: MusicRequest) => { saveCount++; return origSave(r); };
  ...
  test("marca request como refunded quando status é refunded", async () => {
    ...
    const useCase = new ProcessPaymentNotificationUseCase(
      reqRepo as any, showRepo as any, new MockCredsRepo() as any,
      new MockRegistry(buildMockGateway("refunded")) as any, mockLogger as any
    );
