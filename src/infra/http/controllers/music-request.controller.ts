...
import type { CancelMusicRequestUseCase } from "../../../application/use-cases/cancel-request.use-case";
import type { GetShowRequestsUseCase } from "../../../application/use-cases/get-show-requests.use-case";
...
export const musicRequestController = (
  ...
  new Elysia({
    ...
    .post(
      "/:showId/requests",
      async ({ params, body, cookie: { customer_sid } }) => {
    ...
    .patch(
      "/:showId/songs/:songId/play",
      async ({ params, getArtistId }) => {
        const artistId = await getArtistId();
        await markSongAsPlayedUseCase.execute({
          showId: params.showId,
    ...
    .patch(
      "/:showId/requests/:requestId/cancel",
      async ({ params, getArtistId }) => {
        const artistId = await getArtistId();
