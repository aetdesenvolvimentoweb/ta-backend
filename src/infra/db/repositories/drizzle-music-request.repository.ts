...
import { artists, musicRequests, shows, songs } from "../schema";
...
export class DrizzleMusicRequestRepository implements IMusicRequestRepository {
  ...
  async findByPaymentId(paymentId: string): Promise<MusicRequest | null> {
    const [row] = await db
      .select()
      .from(musicRequests)
      .where(eq(musicRequests.paymentId, paymentId));
    return row ? rowToRequest(row) : null;
  }
  ...
  async findBySessionId(showId: string, sessionId: string): Promise<MusicRequest[]> {
    const rows = await db
      .select()
      .from(musicRequests)
      .where(and(eq(musicRequests.showId, showId), eq(musicRequests.customerSessionId, sessionId)));
    return rows.map(rowToRequest);
  }
  ...
  async aggregateAppMetrics(): Promise<{
    ...
    const topArtistsRows = await db
      .select({
        name: artists.name,
        revenueCents: sql<number>`COALESCE(SUM(${musicRequests.tipAmountCents}), 0)::int`.as(
          "revenue"
        ),
      })
      .from(musicRequests)
    ...
    return {
      ...
      topArtistsByRevenue: topArtistsRows.map((r) => ({
        name: r.name,
